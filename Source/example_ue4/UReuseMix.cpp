...
DEFINE_LOG_CATEGORY(LogUReuseMix);

UReuseMix::UReuseMix(const FObjectInitializer& ObjectInitializer)
    ...
    , ItemCount(0)
    , MaxPos(0)
    , Style1(EReuseMixStyle::Vertical)
    , PreviewCount(5)
    , ItemPoolMaxNum(100)
...
void UReuseMix::NativeConstruct()
{
...
void UReuseMix::NativeTick(const FGeometry& MyGeometry, float InDeltaTime)
{
    ...
    for (int32 i = 0; i < RMMT_Num; ++i) {
        if (MsgMap[i] > 0) {
            --MsgMap[i];
            switch (i) {
            case RMMT_AdjustItem: {
                AdjustItem();
                break;
            }
            case RMMT_FillArrOffset: {
                FillArrOffset();
                break;
            }
            case RMMT_AdjustItemWidgetSize: {
...
void UReuseMix::Reload(int32 __ItemCount)
{
    ...
    if (lzSz.Equals(FVector2D::ZeroVector, 0.0001f)) {
        ViewSize = lzSz;
        ReleaseAllItem();
        MsgMap[RMMT_DoReload] = 1;
        return;
    }
    DoReload();
}

void UReuseMix::Refresh()
{
...
void UReuseMix::RefreshOne(int32 __Idx)
{
...
void UReuseMix::ScrollToStart()
{
    if (ScrollBoxList.IsValid())
        ScrollBoxList->ScrollToStart();
}

void UReuseMix::ScrollToEnd()
{
    if (ScrollBoxList.IsValid())
        ScrollBoxList->ScrollToEnd();
}

void UReuseMix::SetScrollOffset(float NewScrollOffset)
{
    if (ScrollBoxList.IsValid())
        ScrollBoxList->SetScrollOffset(NewScrollOffset);
}

float UReuseMix::GetScrollOffset() const
{
    if (ScrollBoxList.IsValid())
...
void UReuseMix::ClearSpecialSize()
{
    SpecialSizeMap.Empty();
    MsgMap[RMMT_FillArrOffset] = 1;
    MsgMap[RMMT_AdjustItemWidgetSize] = 1;
}

void UReuseMix::AddSpecialSize(int32 __Idx, const FVector2D& __Size)
{
...
FVector2D UReuseMix::GetItemSize(int32 idx)
{
...
void UReuseMix::ScrollUpdate(float __Offset)
{
    ...
    //UE_LOG(LogUReuseMix, Log, TEXT("UReuseMix::ScrollUpdate Offset=%d BIdx=%d EIdx=%d"), Offset, BIdx, EIdx);
...
void UReuseMix::UpdateContentSize(TWeakObjectPtr<UWidget> widget)
{
...
void UReuseMix::RemoveNotUsed(const TArray<int32>& items)
{
...
TWeakObjectPtr<UUserWidget> UReuseMix::NewItem()
{
...
void UReuseMix::ReleaseItem(TWeakObjectPtr<UUserWidget> __Item)
{
    __Item->SetVisibility(ESlateVisibility::Collapsed);
    ItemPool.AddUnique(__Item);
}

void UReuseMix::ReleaseAllItem()
{
    if (CanvasPanelList.IsValid()) {
        for (int32 i = 0; i < CanvasPanelList->GetChildrenCount(); i++) {
            UUserWidget* uw = Cast<UUserWidget>(CanvasPanelList->GetChildAt(i));
...
bool UReuseMix::IsVertical() const
{
    return Style1 == EReuseMixStyle::Vertical;
}

bool UReuseMix::IsInvalidParam() const
{
...
#if WITH_EDITOR
void UReuseMix::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
...
void UReuseMix::OnWidgetRebuilt()
{
    ...
    UWorld* wld = GetWorld();
    if (wld && !wld->IsGameWorld()) {
        wld->GetTimerManager().SetTimerForNextTick(this, &UReuseMix::OnEditReload);
    }
#endif
}

void UReuseMix::SyncProp()
{
    if (ScrollBoxList.IsValid()) {
...
void UReuseMix::AdjustItemWidgetSize()
{
    for (TMap<int32, TWeakObjectPtr<UUserWidget> >::TIterator iter(ItemMap); iter; ++iter) {
        int32 idx = iter->Key;
        TWeakObjectPtr<UUserWidget> wdg = iter->Value;
