...
UENUM(BlueprintType)
enum class EReuseMixStyle : uint8
{
    Vertical,
    Horizontal,
};
...
UCLASS()
class EXAMPLE_UE4_API UReuseMix : public UUserWidget
{
    GENERATED_BODY()

public:

	enum EReuseMixMsgType
	{
		RMMT_AdjustItem,
		RMMT_FillArrOffset,
		RMMT_AdjustItemWidgetSize,
		RMMT_DoReload,
		RMMT_Num,
	};
    ...
    UReuseMix(const FObjectInitializer& ObjectInitializer);
...
protected:
	...
	TSet<int32> FullTypeSet;
    float LastOffset;
    int8 MsgMap[RMMT_Num];
};

DECLARE_LOG_CATEGORY_EXTERN(LogUReuseMix, Verbose, All);